...
public:
	...
	void play() {
		...
		while(p->getVidas() > 0 && !salirJuego && !reiniciar) {
			...
			// --- Lógica Enemigos (CORRECCIÓN FINAL) ---
			if(clock() > tHorda + (CLOCKS_PER_SEC * 0.8)) {
				bool tocaBorde = false;
				
				for(int i=0; i<3; i++) for(int j=0; j<10; j++)
					if(m[i][j]->estaVivo() && (m[i][j]->getX() >= 69 || m[i][j]->getX() <= 2)) tocaBorde = true;
				...
				if(tocaBorde) {
					dy = 1;
					dx = dir * -1; // <--- CLAVE: Al bajar, nos movemos hacia adentro
					dir *= -1; // Cambiamos dirección para el próximo turno horizontal
				}
				
				for(int i=0; i<3; i++) for(int j=0; j<10; j++) {
					m[i][j]->mover(dx, dy);
					if(m[i][j]->estaVivo() && m[i][j]->getY() >= p->getY()) p->quitarVida();
				}
				tHorda = clock();
			}
			...
			bool win = true;
			for(int i=0; i<3; i++) for(int j=0; j<10; j++) if(m[i][j]->estaVivo()) win = false;
			if(win) break;
		}
		
		if(!salirJuego && !reiniciar) gameOver();
	}
	
	bool necesitaReiniciar() { return reiniciar; }
	bool debeSalir() { return salirJuego; }
};
